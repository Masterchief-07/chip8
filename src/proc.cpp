...
void Proc::handleUnknow(const INSTRUCTION &instru)
{
  ...
  throw std::runtime_error(std::format("[PROGRAM] UNNKOWN INSTRUCTION {:#06x}"
                              , instru.getInstruction()
                            )
                        );
}
...
void Proc::handleFx55(const INSTRUCTION &instruction)
{
  ...
  const auto x = instruction.getX();
  const auto regI = this->_regI;
  for (size_t i = 0; i <= x; i++) { this->_memory.at(regI + i) = this->_regV.at(i); }
  // next instruction
  this->incrementPC();
}
void Proc::handleFx65(const INSTRUCTION &instruction)
{
  ...
  const auto x = instruction.getX();
  const auto regI = this->_regI;
  for (size_t i = 0; i <= x; i++) { this->_regV.at(i) = this->_memory.at(regI + i); }
  // next instruction
  this->incrementPC();
}
...
void Proc::writeDisplay(const u8 posX, const u8 posY, const u16 memory_pos, const u8 size)
{
  ...
  for (size_t i = 0; i < size ; i++) {
    ...
    for (size_t j = 0; j < 8; j++) {
      if(const u8 v = 0x80; (data & (v >> j)) != 0)
      {
        const auto x = (posX + j) % DISPLAY_X;
        const auto y = (posY + i) % DISPLAY_Y;
...
void Proc::incrementPC() { this->_PC += 2; }
